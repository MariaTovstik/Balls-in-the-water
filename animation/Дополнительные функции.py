...
class Simulation:
    ...
    def toggle_pause(self): # Переключение режима паузы
        self.is_paused = not self.is_paused
        print(f"Пауза: {self.is_paused}")

    def all_balls_stopped(self): # Проверяем, все ли шары остановили движение
...
class Graphics:
    def __init__(self, root, simulation):  # сохраняем ссылки на главное окно и симуляцию
        ...
        self.ball_info = {}  # словарь для связи ID графического объекта с объектом Ball

        self.bind_ball_events()        # Привязываем события мыши

    def bind_ball_events(self): # Привязка событий мыши к canvas
        self.canv.bind("<Motion>", self.on_mouse_move)
        self.canv.bind("<Leave>", self.on_mouse_leave)

    def on_mouse_move(self, event): # обработка движения мыши
        if self.tooltip:
            self.canv.delete(self.tooltip)
            self.tooltip = None

        # Проверяем, находится ли курсор над каким-либо шаром
        overlapping_items = self.canv.find_overlapping(event.x - 1, event.y - 1, event.x + 1, event.y + 1)

        for item in overlapping_items:
            if item in self.ball_info:
                ball = self.ball_info[item]
                # Создаем подсказку с информацией о размере
                tooltip_text = f"Радиус: {ball.r}px\nМасса: {ball.m}"
                self.tooltip = self.canv.create_text(
                    event.x, event.y - 20,
                    text=tooltip_text,
                    fill="white",
                    font=("Arial", 10, "bold"),
    ...
    def draw(self):
        ...
        self.ball_ids.clear()
        self.ball_info.clear()  # очищаем информацию о связях
        ...
        for ball in self.simulation.balls:  # рисуем шар из симуляции
            ...
            self.ball_info[ball_id] = ball  # сохраняем связь между графическим объектом и объектом Ball
